...
register_nav_menus(
	array(
		...
		'orange-nav'		=> __( 'Orange Menu', 'jointswp' ),		// Utility nav in header
...
// The Utility Menu
function port_discovery_orange_nav() {
	wp_nav_menu(array(
		'container'			=> false,						// Remove nav container
		'menu_class'		=> 'orange-nav medium-horizontal menu',	// Adding custom nav class
		'items_wrap'		=> '<ul id="%1$s" class="%2$s" data-responsive-menu="accordion medium-dropdown">%3$s</ul>',
		'theme_location'	=> 'orange-nav',					// Where it's located in the theme
...
// The Top Menu
function port_discovery_top_nav() {
	wp_nav_menu(array(
		'container'			=> false,						// Remove nav container
...
function port_discovery_social_links() {
	wp_nav_menu(array(
		'container'			=> 'false',				// Remove nav container
...
// The Off Canvas Menu
function port_discovery_off_canvas_nav() {
	wp_nav_menu(array(
		'container'			=> false,							// Remove nav container
...
// The Footer Menu
function port_discovery_footer_links() {
	wp_nav_menu(array(
		'container'			=> 'false',				// Remove nav container
...
function port_discovery_main_nav_fallback() {
	wp_page_menu( array(
		'show_home'		=> true,
...
function port_discovery_footer_links_fallback() {
		...
		if ( $args->theme_location == 'social-links') {
			...
			foreach( $items as &$item ) {
				...
				if( $icon ) {
					...
					$item->title = '<span class="icon" aria-hidden="true"><img class="style-svg" src="' . $icon['url'] . '" alt="' . $icon['alt'] . '"></span><span>' . $item->title . '</span>';
