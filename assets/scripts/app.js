...
(function($) {
    ...
    _app.banner_slider = function() {
        const loopDelay = 7000;
        
        $(window).on('load resize', function() {
            var titleWidths = 0;
            const sliderWidth = $('.banner-home-slider').width();
            const content = $('.banner-home-slider .slide').find('.inner');
            const spacer = $(content).find('.spacer');
            
            $('.banner-home-slider .title:not(.active)').each(function(){
                const $thisWidth = $(this).outerWidth();
                titleWidths += $(this).outerWidth();
            });
        ...
        // AutoPlay Slide On Click    
        function timer(interval = loopDelay) {
            function loop() {
            
                if( $('.banner-home-slider').hasClass('done') ) {
                    // do nothing 
                } else {
                setTimeout(loop, interval);
                }
                
                $(".title.active:not(.clicked)").next().next('.title').addClass("active").siblings('.title').removeClass('active');
                
                $(".active:not(.clicked)").prev().removeClass('closed').siblings('.slide').addClass('closed');
        
                const lastItem = $('.title:last')
                if( $(lastItem).hasClass('active') ) {
                    $('.banner-home-slider').addClass('done');  
                }            
            }
            loop();
        }
        ...
        $('.title').click(function () {
            ...
            $(this).addClass("active clicked").siblings('.title').removeClass('active');
            
            $(this).prev().removeClass('closed').siblings('.slide').addClass('closed');
            
            setTimeout(() => {
                $(".title.active").removeClass('clicked');
            }, loopDelay);
